    ...
    def _impl(*args, **kwargs):
        ...
        if devchk_list:
            ...
            if tensor_args:
                ...
                if device is None:
                    device = getattr(input_tensor, '_tt_device', None)
        ...
        for i,x in enumerate(args):
            if isinstance(x, Tensor):
                ...
                if optype in ['Add', 'Sub', 'Mul']:
                    ...
                    exit(0)
            elif 'SimTensor' in str(type(x)):  # Handle SimTensor
                x.op_in.append(op_name)
                opinfo['inList'].append(x.name)
                new_args.append(x)
            else:
        ...
        if device is not None:
            device.add_op(opobj)
        # Agar None hai, toh check karo Output Tensor 'C' ke paas device hai kya
        elif hasattr(C, 'device') and C.device is not None:
            C.device.add_op(opobj)
        else:
            pass  # Final fallback: Sirf warning do, crash mat karo
           # print(f"WARNING: No valid device found for {optype}, skipping add_op tracking")
        # --- FIX ENDS HERE ---
...
def multiple_output_immediate_op(optype, /, preprocess=None):
    def _impl(*args, **kwargs):
        ...
        tensor_args = [x for x in args if isinstance(x, Tensor) or 'SimTensor' in str(type(x))]
        devchk_list = [x.device for x in tensor_args]
        device      = devchk_list[0]
        # assert device and all(x == device for x in devchk_list), f"device check: {devchk_list}"
        ...
        new_args = []
        for i,x in enumerate(args):
            if isinstance(x, Tensor) or 'SimTensor' in str(type(x)):
                x.op_in.append(op_name)
                opinfo['inList'].append(x.name)
                new_args.append(x)
            else:
...
def reshape_pp(args_list, kwargs_dict):
    ...
    assert isinstance(inT, (Tensor, SimTensor)), f"ttnn.reshape 1st input should be a ttnn.Tensor"
    ...
    if (len(args_list) == 3):
        ...
        # Handle SimTensor vs regular Tensor
        if hasattr(inT, "device"):
            inT = Tensor(shape=outShape, device=inT.device, dtype=in_dtype)
        else:
            # SimTensor case - create new SimTensor with reshaped dimensions
            inT_cfg = {"name": f"{inT.name}_reshaped", "shape": list(outShape), "dtype": inT.dtype}
            inT = SimTensor(inT_cfg)
    ...
    # Create shape tensor - handle SimTensor case
    if hasattr(inT, "device"):
        shapeT = Tensor(shape=shapeData.shape, dtype=DataType.INT64, device=inT.device, data=shapeData)
    else:
        # SimTensor case - create shape tensor without device
        shapeT = SimTensor({"name": "shape_tensor", "shape": list(shapeData.shape), "dtype": np.int64, "data": shapeData})
