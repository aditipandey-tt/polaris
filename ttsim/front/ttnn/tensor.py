...
class Tensor(SimTensor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for kk,cls in typechecks.items():
            if kk in kwargs:
                obj = kwargs[kk]
                if kk == 'device' and obj is None:
                    # Skip device check for None - some ops after CCL don't have device
                   continue
                else:
        ...
        # Force list conversion if it's a Shape object
        if hasattr(obj, '_shape'):
            obj = list(obj._shape)
        
        # Ab check karo (Assertion ko thoda relax kar diya)
